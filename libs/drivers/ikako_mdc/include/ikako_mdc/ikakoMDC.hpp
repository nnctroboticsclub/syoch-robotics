...
template <typename T, typename MotorT = robotics::node::ikakoMDCMotor<T>>
class ikakoMDCPair : public MotorPair<T, T, MotorT> {
 ...
 public:
  ikakoMDCPair(::ikakoMDC& mdc)
      : motor_(new MotorT(mdc)), encoder_(new node::ikakoMDCEncoder<T>(mdc)) {}

  void Update() { encoder_->UpdateNode(); }

  Node<T>& GetEncoder() override { return *encoder_; }

  MotorT& GetMotor() override { return *motor_; }
};
}  // namespace robotics::assembly
